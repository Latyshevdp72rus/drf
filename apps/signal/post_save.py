...
def log_book_save(sender, instance, created, **kwargs):
    if created:
        logging.warning(f"В таблице Book, появилась новая запись  {instance.book_name}  и она успешно создана")
...
def log_author_save(sender, instance, created, **kwargs):
    if created:
        logging.warning(f"В таблице Author, появилась новая запись {instance.first_name} {instance.last_name} {instance.fathername} и она успешно создана")
